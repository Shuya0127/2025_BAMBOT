...
  
  useEffect(() => {
    const isNewPosition = center[0] !== INITIAL_POSITION[0] || center[1] !== INITIAL_POSITION[1];
    
    ...
    useEffect(() => {
        if (markerRef.current && !isNaN(position[0]) && !isNaN(position[1])) {
            ...
            if (popup) {
                ...
                popup.setContent(`最終受信座標: <br />緯度: ${lat.toFixed(6)} <br />経度: ${lng.toFixed(6)}`);
            }
        }
    }, [position]); 
...
// GpsMap は position を props から受け取る
const GpsMap = ({ position }) => { 
  
  // WebSocketの処理ロジックは削除されました
